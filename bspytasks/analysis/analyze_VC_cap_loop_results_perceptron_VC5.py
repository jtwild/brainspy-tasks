...
# %% User data
base_dir = r'C:\Users\Jochem\STACK\Daily_Usage\Bestanden\UT\TN_MSc\Afstuderen\Results\Electrode_importance\2020_04_21_capacity_loop_hyperparameters_VC5\perceptron_data\1000epochs'
loop_items_file = r'C:\Users\Jochem\STACK\Daily_Usage\Bestanden\UT\TN_MSc\Afstuderen\Results\Electrode_importance\2020_04_21_capacity_loop_hyperparameters_VC5\loop_items.npz'
glob_filter = '*.npz'
# %% Remaining script
# Load descriptions
...
equal_array[:,0] = False # if this is the only false in the list, then perceptron could not make it better after first epoch, so it was 100% at once.
for i in trange(nr_files):
    converged[i] = np.where(equal_array[i,:] == False)[0][-1] # the first return element: [0] is the index, and then we want the last one: [-1]. SO th last index where data was imrpvoed.
plt.figure()
