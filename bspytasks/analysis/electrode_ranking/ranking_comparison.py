...
fig_b, ax_b = plt.subplots(nrows=2, ncols=4, sharey=True)
ax_b = ax_b.flatten()
for i in range(n_models):
...
fig_d, ax_d = plt.subplots(nrows=2, ncols=4, sharey=True)
ax_d = ax_d.flatten()
for i in range(n_elec):
...
# Question to be answered: For a specific electrode, which model get's highest ranking, and do the methodes agree?fig_d, ax_d = plt.subplots(nrows=2, ncols=4, sharey=True)
