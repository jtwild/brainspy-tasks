...
loss_fns = ['sigmoid_nn_distance']*40
batch_sizes = [4]
...
regularizers = [[76,-340]]             # not be looped seperately, but combine with the above


base_configs = load_configs('configs/tasks/filter_finder/template_ff_gd_pretrained_inputs.yaml')
loop=0
        ...
        for i in range(len(types)):
            for batch in batch_sizes:
                for loss in loss_fns:
                    ...
                    configs['filter_finder']['algorithm_configs']['processor']['processor_type'] = types[i]
