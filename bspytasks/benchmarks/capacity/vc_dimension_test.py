...
class VCDimensionTest():
    ...
    def __init__(self, configs):
        self.init_algorithm(configs['algorithm'])
        self.init_filenames(configs['output_dir'])
        self.init_excel_writer()

    def init_algorithm(self, configs):
        self.algorithm = get_algorithm(configs['type'], configs['configs_path'])

    def init_filenames(self, output_dir):
        self.output_dir = output_dir
        self.surrogate_model_name = os.path.basename(os.path.splitext(self.algorithm.get_torch_model_path())[0])
    ...
    def init_data(self, inputs, binary_labels, threshold):
        column_names = ['gate', 'found', 'accuracy', 'max_fitness', 'best_output', 'best_genome']
    ...
    def __test_label(self, inputs, label, threshold):
        ...
        if len(set(label)) == 1:
            ...
            results = {}
            results['best_genome'] = np.nan
            results['best_output'] = np.nan
            results['max_fitness'] = np.nan
            results['accuracy'] = np.nan
            # genes, output, fitness, accuracy = np.nan, np.nan, np.nan, np.nan
            found = True
        else:
            print('Finding classifier ', label)
            results = self.algorithm.optimize(inputs, label)
            found = (results['accuracy'] > threshold)
    ...
    def save_plot(self, threshold, dimension, show_plot):  # pylint: disable=E0202
        plt.figure()
        fitness_classifier = self.data['max_fitness'].to_numpy()
